...
既定では `127.0.0.1` にのみ bind します。既定ポートは `47117`（`chat/config.json` で変更可）。別の端末（同じ tailnet の iPhone など）から使うときは待ち受けアドレスを足せます（§5.6）。
...
- システムプロンプトはジョブ cwd の `AGENTS.md`（`prompts/system.md` へのリンク）として読ませます。Codex は cwd の `AGENTS.md` を自動で読みます。Claude の `--system-prompt-file` と違い、これは Codex 自身のコーディング向けの指示に足す形です。それでも相談役として振る舞い、skill も使いました（V15）。
- Codex の基本指示を置き換える設定 `-c model_instructions_file=<file>` はありますが、使いません。1 ターンあたり約 6,000 トークン軽くなる代わりに、ツールの使い方やサンドボックスの制約に関する指示も消えるためです（V15）。
...
- **Host の検査**: `Host` ヘッダーが待ち受け中のアドレスか `localhost`（どちらもポート込み）でなければ 403 にします。DNS リバインディング（攻撃者のドメインを 127.0.0.1 に向け直す）で同一オリジン扱いの GET を通されるのを防ぎます。
- **待ち受けアドレス**: 既定は `127.0.0.1` だけです。`chat/config.json` の `extraBindHosts` か環境変数 `CHAT_BRIDGE_HOSTS`（カンマ区切り）でアドレスを足すと、そのアドレスごとに listener を作ります。0.0.0.0 での一括 bind はしません。足したアドレスから来るページの Origin は `CHAT_ALLOWED_ORIGINS`（カンマ区切り）で許可します。個人のアドレスを git に入れずに済むよう、環境変数を用意しています。tailnet のアドレスを足した場合、同じ tailnet の端末からブリッジ API に到達できます。守りはトークン・Origin 許可リスト・Host 検査の 3 つです（§16）。
...
スナップショットの 3 つは、全国図鑑 No. の範囲で書いて小さく保ちます。`25` は 1 種、`25-30` は連番で、出てこない No. は記録なしです。記録が全体の半分を超える項目については、「No.25 以外のすべて」のように少ない側だけを書きます。`routes.md` では、作品ごとに入手方法と条件が同じものを 1 行にまとめました。名前は載せず、必要なら `reference/index.md` と `reference/species/<4桁のNo>.md` で引かせます。

この形にした理由は入力トークンです。1 種 1 行の表にしていたときは `collection.md` が 33,876 バイト（約 2 万トークン）あり、その大半は記録が何も無い約 1,000 種の行でした。圧縮後は `collection.md` が約 1,900 バイト、`routes.md` が 6,378 バイト（全ソフト所持・未捕獲の場合）、`bank.md` が約 1,300 バイトです。`reference/index.md` は約 23,000 文字、種別ファイルは 2〜5 KB で、どれも 1 回の Read に収まります。
...
`data/` の 3 文書は、収集状況に触れる質問のたびに読まれます。大きさがそのまま毎回の入力トークンになるので、次のルールで書きます。実装は `lib/chat/compact.ts` です。

| id | ルール | 例 |
|---|---|---|
| R1 | 既定値は書かない。省いたものの意味は文書の冒頭で 1 度だけ宣言する | 「ここに出てこない No. は記録なし（未捕獲）です」 |
| R2 | 連番は範囲に畳む | `25, 26, 27` → `25-27` |
| R3 | 対象が母集合の半分を超えたら補集合で書く | `- 捕獲済み（1024匹）: No.25 以外のすべて` |
| R4 | 属性が同じ項目は 1 行にまとめ、対象を id の並びで持つ | `- 固定シンボル（★3）: No.25, 27-29` |
| R5 | 常には要らない情報（名前・出典・条件の原文）は文書に入れず、`reference/` のパスの引き方だけ書く | `名前と No. の対応は reference/index.md` |
| R6 | 数えれば分かることでも、よく聞かれる集計は先頭に置く | 残り数、世代別の未捕獲、分類別の未捕獲 |
| R7 | 略号と見出しは固定文字列にし、skill の説明と一致させる | 「記録」「未捕獲」「No.」 |

R5 で外に出す先は 2 つです。`reference/index.md` は No. と名前の対応、`reference/species/<4桁のNo>.md` は 1 種ぶんの詳細で、どちらも必要になった質問でだけ読まれます。

R3 の反転は、収集が進んだ人ほど効きます。全種を捕獲した状態では `- 捕獲済み: 全 1025 種` の 1 行です。

#### ルーチン

- **予算**: スナップショットの 1 文書あたり 8,000 バイト（`SNAPSHOT_BUDGET`）。`reference/` の 1 ファイルは Read の上限（25,000 トークン）に収まる大きさとし、索引は約 23,000 文字です。
- **検証**: `scripts/check-engine.mjs` が「記録なし・少数・半分・全部」の 4 状態で 3 文書を生成し、予算とルール（R1〜R4）を確かめます。予算を超えるとテストが落ちます。
- **文書の形を変えるとき**: 生成関数（`lib/chat/collection.ts`・`plan.ts`・`reference.ts`）、skill の読み方（`chat/skills/*/SKILL.md`）、`scripts/check-engine.mjs` の assert、本節と §6.2 を同じ PR で直します。skill の説明と実物がずれると、AI は読めても解釈を誤ります。
- **効果の測り方**: 実際に質問を 1 回投げ、`~/.claude/projects/<cwd を変換した名前>/<セッション>.jsonl` を開きます。`tool_result` の文字数が読ませた量、`message.usage` の `cache_creation_input_tokens` がその回の実質的な入力です。1 種 1 行の表だったときは 27,542 文字・約 2 万トークン、圧縮後は 1,509 文字でした。
- **固定費**: 上の測り方で見ると、質問の内容と無関係に約 9,100 トークンが毎回かかります。内訳は Claude Code のシステムプロンプト、ツール定義、skill 20 個の frontmatter です（§16 のリスク 5）。文書側の圧縮ではこれ以上減りません。
...
ドックの接続先は、ページのホストから決めます。`localhost` / `127.0.0.1` / `file://` と、デプロイ済みの https サイトからの接続先は `http://127.0.0.1:<port>` です。それ以外のアドレスから http で開いたページ（tailnet のアドレスなど）は、同じアドレスの同じポートを見ます。ページを配っている Mac の上でブリッジも動いているためです。
...
接続できたが `providers[].available` が false の AI は選択肢を無効化し、`reason` に応じて「CLI が見つかりません」「未対応（フェーズ 2）」と表示します。`/health` が 403 `origin_not_allowed` のときの案内は 2 通りで、`chat/config.json` の `allowedOrigins` に追加する Origin を示すか、`file://` のページ（送る Origin は `null`）向けに `allowNullOrigin` を `true` にするよう促す文面です。トークンが未入力・不一致ならトークン入力欄を出します。
...
- `extraBindHosts` は `127.0.0.1` に足す待ち受けアドレスです（既定は空）。一時的に足すだけなら環境変数 `CHAT_BRIDGE_HOSTS` と `CHAT_ALLOWED_ORIGINS` を使います（§5.6）。
...
| V15 | Codex に `AGENTS.md` だけで相談役として振る舞わせられるか。`model_instructions_file` で置き換えた場合との違い | **振る舞う**。同じ発言（画面と版のヘッダー＋「ピカチュウは捕まえた？HOMEには送った？」）を 2 通りで投げ、どちらも skill を読んでから `data/collection.md` を読み、「捕獲済みだが HOME 送信の記録はない」と答えた。コーディングエージェントらしい脱線は出なかった。置き換えなしは入力 31,781・出力 432 トークン、置き換えありは入力 25,431・出力 242 トークン | 既定は `AGENTS.md` のまま（§5.3）。置き換えは採用しない |
...
1. **V6 が通らないとサイト版でチャットが使えない（成立条件）**。ブラウザが `https://` ページから `http://127.0.0.1` への fetch を止めると、ブリッジ方式はサイト版で成り立ちません。Chrome 152 は許可ダイアログ経由で通ることを確認しました（V6）。Safari は未確認のまま対象外とし、`docs/operations.md` とドックの未接続画面に「対応ブラウザは Chrome」と書きます。通らなかった場合の代替は、(a) ブリッジを `https://127.0.0.1` にして自己署名証明書を信頼させる、(b) サイト版ではチャットを無効にし `pnpm dev` と単一 HTML 版（`allowNullOrigin: true`）に限定する、の 2 つです。実装前に Chrome と Safari で `fetch('http://127.0.0.1:47117/health')` を試して決めます。
...
6. **待ち受けアドレスを足すと、その範囲の端末からブリッジ API に届く**。tailnet のアドレスを足した場合、同じ tailnet の端末はトークンさえあれば質問を投げられます。トークンは 32 文字で、Origin 許可リストと Host 検査も通る必要があります。ブリッジは 0.0.0.0 では待ち受けないので、足していないネットワーク（公衆 Wi-Fi など）からは届きません。
7. **`~/.claude/projects/` にジョブごとのディレクトリが増える**（V13）。CLI はセッションを cwd ごとのディレクトリに保存するため、質問 1 回につき 1 つ増えます。消すと `--resume` が効かなくなり転写方式に落ちる（§9.1）ので、ブリッジは消しません。
...
- ブラウザが作る文書: `scripts/check-engine.mjs` で `collection.md` の集計と No. 範囲の書式（§6.5 の予算 8,000 バイトを含む）、`routes.md` の作品別の見出しと捕獲済みの除外、`bank.md` の印付き一覧、種別ファイルの進化系統と入手方法の表を確認します。
- フロント: 既存の `pnpm test` は engine 確認のみなので、`ChatDock` は次の手動確認で見ます。
  - [ ] 開閉・幅のドラッグ（320〜640px）が、ページを移動しても保たれる
  - [ ] 生成中に左ナビで別ページへ移動しても、本文が欠けず二重にもならずに続きから表示される
  - [ ] 入力途中の文がページ移動後も残る
  - [ ] 捕獲チェックを付けた直後の質問に、新しい収集状況で答える
  - [ ] サイト版と単一 HTML 版を同時に開いても、互いの収集状況を上書きしない（保存元が別の版になる）
  - [ ] 履歴から再開・削除（確認ダイアログあり）ができる
  - [ ] ブリッジ停止中は起動コマンドとトークン入力の案内が出て、生成中にブリッジが止まった回答は「ブリッジが停止したため中断しました」になる
  - [ ] 640px 以下では下からのシートで開く
...
- `lib/chat/plan.ts`、`lib/chat/reference.ts`、`lib/chat/models.ts`、`lib/chat/compact.ts`
...
- `docs/operations.md`: 起動手順・設定・制約（README はリポジトリの紹介だけにし、ここへ移しました）
